        ...
        public static void tooltip(ItemTooltipEvent event) {
            ...
            if (stack.getItem() instanceof net.exmo.exmodifier.content.modifier.EntryItem) {
                if (!event.getToolTip().isEmpty()) {
                    ...
                    lc.add(new TranslatableComponent("modifiler.entry." + modifierId.substring(2)));
                    if (!Screen.hasShiftDown()) {
                        ...
                        lc.add(new TranslatableComponent("modifiler.entry.possibility").append(df.format(possibility)).append("%"));

                        if (!modifierType.isEmpty())
                            lc.add(new TranslatableComponent("modifiler.entry.type").append(new TranslatableComponent(modifierType)));
                        lc.add(new TextComponent(" "));
                        lc.add(new TranslatableComponent("modifiler.entry.look_more_shift"));
                    }else {
                        ModifierEntry modifierEntry = ModifierHandle.modifierEntryMap.get(modifierId);
                        if (modifierEntry==null)lc.add(new TranslatableComponent("modifiler.entry.unknown_modifier"));

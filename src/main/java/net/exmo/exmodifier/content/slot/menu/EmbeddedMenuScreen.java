...
@OnlyIn(Dist.CLIENT)
public class EmbeddedMenuScreen extends AbstractContainerScreen<EmbeddedMenu> {
    ...
    private static final ResourceLocation ENCHANTING_TABLE_LOCATION = new ResourceLocation("exmodifier:textures/gui/container/embedded_table.png");
    ...
    public EmbeddedMenuScreen(EmbeddedMenu p_98754_, Inventory p_98755_, Component p_98756_) {
        super(p_98754_, p_98755_, p_98756_);
        this.player = p_98755_.player;
    }
    ...
    protected void renderBg(GuiGraphics p_282430_, float p_282530_, int p_281621_, int p_283333_) {
        ...
        for(int l = 0; l < 3; ++l) {
            ...
            if (k1 == 0) {
                ...
                int i2 = 6839882;
                if (((k < l + 1 || this.minecraft.player.experienceLevel < k1) && !this.minecraft.player.getAbilities().instabuild) || this.menu.ClueId[l] == -1) { // Forge: render buttons as disabled when enchantable but enchantability not met on lower levels
                    p_282430_.blit(ENCHANTING_TABLE_LOCATION, i1, j + 14 + 19 * l, 0, 185, 108, 19);
    ...
    public void render(GuiGraphics p_283462_, int p_282491_, int p_281953_, float p_282182_) {
        ...
        for(int j = 0; j < 3; ++j) {
            ...
            int finalJ = j;
            var id = this.menu.ClueId[finalJ];
            ModifierSlot a = (ModifierSlot) BaseItemSelected.getValue(id);// 如果找不到匹配项，则返回 null
            if (a==null)continue;
            int l = (this.menu).levelClue[j];
            int i1 = j + 1;
            if (this.isHovering(60, 14 + 19 * j, 108, 17, (double)p_282491_, (double)p_281953_) && k > 0) {
                List<Component> list = Lists.newArrayList();
                String string = ModifierSlotHandle.getKey(a).toString();
                list.add((Component.translatable("modifier.slot."+ string)).withStyle(ChatFormatting.WHITE));
                if (a == null) {
